    ...
    const fetchMessages = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/direct-message/${currentUser}/${selectedUser}`
        );
        console.log("hello");
    ...
    if (message.trim()) {
      try {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { sender: currentUser, message, timestamp: new Date() },
        ]);
        setMessage("");
      } catch (error) {
        ...
        {messages.length > 0 ? (
          messages.map((msg, index) => (
            <div
              key={index}
              className={`message ${
                msg.sender === currentUser ? "current-user" : "other-user"
              }`}
            >
              <strong
                className={`user-name ${
                  msg.sender === currentUser ? "current" : "other"
                }`}
              >
                {msg.sender}
              </strong>
