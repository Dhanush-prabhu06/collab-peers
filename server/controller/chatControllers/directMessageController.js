import DirectMessage from "../../model/directMessageModel.js";
...
export const getDirectMessages = async (req, res) => {
  ...
  try {
    ...
    if (!conversation) {
      return res
        .status(404)
        .json({ error: "No conversation found between these users." });
    }
...
export const getDirectMessagesByOneUser = async (req, res) => {
  const { user1, user2 } = req.params;
...
export const sendDirectMessage = async (req, res) => {
  ...
  console.log(
    `user1: ${user1}, user2: ${user2}, sender: ${sender}, message: ${message}`
  );

  if (sender !== user1 && sender !== user2) {
    return res
      .status(400)
      .json({ error: "Sender must be one of the participants." });
  }
